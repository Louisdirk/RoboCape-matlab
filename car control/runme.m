...
model = ModelRealCar(...                  % Car model, state vector: [position x; position y; forward velocity; heading; steering angle]
    ...
    'Parameters',[(1/0.6);(1/0.6);1;1]... % Parameters first order models (xDot = -k*(x-g*u)):  [k velocity; k steering angle; g velocity; g steering angle;]
    );
...
mode = 2;
Rnoise = 0.1^2*eye(2);
Qnoise = 10*diag(([0.01;0.01;0.04;pi/3;pi/2]/3).^2);
switch mode
    case 1
        sys = model;
...
sys.controller = InlineController(@(t,x)[1;0.3]);
